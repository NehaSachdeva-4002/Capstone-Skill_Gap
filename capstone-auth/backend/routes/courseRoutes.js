...
router.post('/recommendations', async (req, res) => {
  try {
    ...
    console.log('Getting course recommendations for skills:', skills);
    
    const recommendations = CourseService.getRecommendationsForSkills(skills);
    
    res.json({
      success: true,
      recommendations,
      skillsCount: skills.length,
      totalCourses: Object.values(recommendations).reduce((sum, rec) => sum + rec.courses.length, 0)
    });
  } catch (error) {
...
/**
 * POST /api/courses/knowledge-graph
 * Generate knowledge graph data for skills
 */
router.post('/knowledge-graph', async (req, res) => {
  try {
    ...
    console.log('Generating knowledge graph for skills:', skills);
    
    const graphData = CourseService.generateKnowledgeGraphData(skills);
...
router.get('/skill/:skillName', async (req, res) => {
  try {
    ...
    console.log('Getting courses for skill:', skillName);
    
    const courses = CourseService.getCoursesForSkill(skillName);
    ...
    console.error(`Error getting courses for ${req.params.skillName}:`, error);
    res.status(500).json({
...
router.get('/', async (req, res) => {
  try {
    ...
    // Flatten all courses from the database
    const allCourses = [];
    Object.values(CourseService.COURSE_DATABASE).forEach(courseArray => {
    ...
    res.json({
      ...
      skillsAvailable: Object.keys(CourseService.COURSE_DATABASE).length
    });
  } catch (error) {
...
router.get('/health', (req, res) => {
  res.json({
    ...
    coursesAvailable: Object.keys(CourseService.COURSE_DATABASE).length
  });
});
